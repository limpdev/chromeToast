...
const zipName = `toast-${browserTarget}.zip`;
...
function createManifest(baseManifest, target) {
...
function copyManifest() {
  ...
  const manifest = createManifest(baseManifest, browserTarget);
  fs.writeFileSync(
...
function startSseServer() {
  ...
  server.listen(SSE_PORT, () => {
    console.log(
      `[dev] SSE server listening on http://localhost:${SSE_PORT}/events`,
    );
  });
}
...
async function runDev() {
  ...
  console.log(
    "[dev] Watching for changes. Load dist/ as an unpacked extension.",
  );
